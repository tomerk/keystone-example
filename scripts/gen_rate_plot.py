...
ax = sns.boxplot(x="pos_in_partition", y="rate", color=bandit_color, data=df)
...
df = df[df['pos_in_partition'] == 0].copy()
df['partition_start_time'] = df['system_nano_start_time']
bandit_rewards = pd.merge(bandit_rewards, df[['policy', 'partition_id', 'partition_start_time']], on=['partition_id', 'policy'])
